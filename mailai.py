...
def build_prompt(msgs: list[dict], facts, tags, tzname: str = "") -> str:
    ...
    p += [
        ...
        "  · 链接指向的域名和正文说的完全不是一回事",
        # 实测又逮到一类:微软从 accountprotection.microsoft.com 发来的密码
        # 更改通知,因为"里面说的那个账号不是我的"被判成了诈骗。可那是**真**
        # 通知,域名就是微软的 —— 这种该是系统通知,不是骗局
        "**发件域名确实属于它自称的那家机构的,一律不算诈骗** —— 哪怕里面说",
        "的账号不是我的、内容莫名其妙。那种打「系统通知」,summary 里说清",
        "「这不是你的账号,不用管」就行。域名对不上才是实据。",
        ...
        "",
        "每封**至少一个标签**,最多三个。why 一句话说清为什么是这个 level。",
        "",
        # summary 单独拎出来说。原来那句是「说清这封信要我干什么」,于是模型
        # 写出来的是一堆"需要你在 X 前回复"「无需处理」"建议关注" —— 判断
        # 有没有用、要不要动手,用户自己会;他要的只是**这封信是干什么的**。
        "summary 就写**这封信在说什么**,一句话,直白到不用再想:",
        "  · **不要**加「需要你…」「建议…」「无需处理」「值得关注」这类判断和",
        "    催促。有没有用他自己会看",
        "  · 不要复述主题,也不要客套(「您好」「此邮件旨在」)",
        "  · 有具体的数字、日期、金额、地点就**写进去** —— 摘要的价值全在这儿。",
        "    「9/21 10 点前交表单」比「提醒你尽快完成」有用一百倍",
        "  · **验证码 / 一次性密码要直接写在最前面。** 这种信的全部价值就是那",
        "    几位数字,让人再点开一次是白费事。写成「验证码 123456」开头,",
        "    后面再补一句是哪个服务的。右下角那条弹窗直接显示 summary,",
        "    所以写对了就等于不用开邮箱了",
        "  · 快递、账单、订单同理:单号、金额、送达时间写进去",
        "",
