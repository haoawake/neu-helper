...
抽错了点那一格就能改；改过的条目不会被下一次解析冲掉，删掉的也不会被下一次解析
请回来。**但删掉的不是丢掉** —— 日程页底下会有一条「N 条被你删掉了」，点「恢复」
就回来。这一条是补上来的：条目 id 按内容算，重新解析回来的还是同一个 id、照样被
那条删除记录压住，而它在格子里又是彻底看不见的 —— 于是从用户那头看就是「点多少次
